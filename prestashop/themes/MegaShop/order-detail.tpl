...
{if $return_allowed}
	<div id="returnOrderMessage">
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="button">{l s='Make an RMA slip'}<i class="fa fa-chevron-right right"></i></button>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="button">{l s='Send'}<i class="fa fa-chevron-right right"></i></button>
		</div>
	</form>
{else}
