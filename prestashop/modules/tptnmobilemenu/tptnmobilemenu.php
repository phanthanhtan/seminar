...
class TptnMobileMenu extends Module
{
	...
	public function install()
	{
		if (!parent::install() ||
			...
			!Configuration::updateGlobalValue('MOD_TPTNMOBMENU_ITEMS', 'CAT3,CAT4') ||
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitTptnmobilemenu'))
		{
			...
			if (is_array($items) && count($items) && Configuration::updateValue('MOD_TPTNMOBMENU_ITEMS', (string)implode(',', $items)))
		...
		else if (Tools::isSubmit('submitTptnmobilemenuLinks'))
		{
			...
			if ($count_links_label || $count_label)
			{
				...
				{
					MobileMenu::add($links_label, $labels,  Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
					$this->_html .= $this->displayConfirmation($this->l('The link has been added.'));
				}
			}
			$update_cache = true;
		}
		elseif (Tools::isSubmit('deletetptnmobilemenu'))
		{
			$id_tptnmobilemenu = Tools::getValue('id_linksmenutop', 0);
			MobileMenu::remove($id_tptnmobilemenu, (int)Shop::getContextShopID());
		...
		elseif (Tools::isSubmit('updatetptnmobilemenu'))
		{
			$id_tptnmobilemenu = (int)Tools::getValue('id_linksmenutop', 0);
			...
			if (Tools::isSubmit('updatelink'))
			{
				...
				MobileMenu::update($link, $label, $new_window, (int)$id_shop, (int)$id_tptnmobilemenu, (int)$id_tptnmobilemenu);
		...
		$links = MobileMenu::gets((int)$id_lang, null, (int)Shop::getContextShopID());
	...
	private function getMenuItems()
	{
		...
		{
			$conf = Configuration::get('MOD_TPTNMOBMENU_ITEMS');
			if (strlen($conf))
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = MobileMenu::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = MobileMenu::get($link[0]['id_linksmenutop'], (int)$default_language, (int)Shop::getContextShopID());
						}
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = MobileMenu::get($link[0]['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
						}
	...
	public function hookActionShopDataDuplication($params)
	{
		...
		{
			Db::getInstance()->execute('
				INSERT IGNORE INTO '._DB_PREFIX_.'tptnmobilemenu (id_linksmenutop, id_shop, new_window) 
				VALUES (null, '.(int)$params['new_id_shop'].', '.(int)$link['new_window'].')');
			
			$tptnmobilemenu[$id]['new_id_linksmenutop'] = Db::getInstance()->Insert_ID();
		}
		...
		foreach($tptnmobilemenu as $id => $link)
		{
			...
			foreach($lang as $l)
				Db::getInstance()->execute('
					INSERT IGNORE INTO '._DB_PREFIX_.'tptnmobilemenu_lang (id_linksmenutop, id_lang, id_shop, label, link) 
	...
	public function renderAddForm()
	{
		$fields_form = array(
			'form' => array(
				'legend' => array(
					'title' => (Tools::getIsset('updatetptnmobilemenu') && !Tools::getValue('updatetptnmobilemenu')) ? $this->l('Update link') : $this->l('Add a new link'),
					'icon' => 'icon-link'
				),
				'input' => array(
					array(
				...
				'submit' => array(
					'name' => 'submitTptnmobilemenuLinks',
					'title' => $this->l('Add')
				)
			),
		);
		...
		if (Tools::getIsset('updatetptnmobilemenu') && !Tools::getValue('updatetptnmobilemenu'))
			$fields_form['form']['submit'] = array(
				'name' => 'updatetptnmobilemenu',
				'title' => $this->l('Update')
			);

		if (Tools::isSubmit('updatetptnmobilemenu'))
		{			
	...
	public function renderChoicesSelect()
	{
		...
		$links = MobileMenu::gets($this->context->language->id, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$default_language = Configuration::get('PS_LANG_DEFAULT');
				$link = MobileMenu::get($link['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
	...
	public function getAddLinkFieldsValues()
	{
		...
		if (Tools::isSubmit('updatetptnmobilemenu'))
		{
			$link = MobileMenu::getLinkLang(Tools::getValue('id_linksmenutop'), (int)Shop::getContextShopID());
	...
	{
		$links = MobileMenu::gets((int)$this->context->language->id, null, (int)Shop::getContextShopID());
		$fields_list = array(
